...
The Self-Corrective RAG framework is composed of multiple specialized pipelines, each responsible for a distinct stage of the Retrieval-Augmented Generation lifecycle. Instead of implementing all logic inside a single workflow, the system separates document ingestion, memory management, retrieval, generation, evaluation, and recovery into independent modules coordinated by **LangGraph**.

This modular architecture improves maintainability, testability, scalability, and allows every subsystem to evolve independently.
...
The Ingestion Pipeline transforms raw documents into high-quality vector embeddings that can be efficiently searched during retrieval. It serves as the knowledge acquisition layer of the system, ensuring that every document is cleaned, enriched, chunked, embedded, and stored in the vector database.

Without this pipeline, the retrieval system would have no searchable knowledge base.

---

## 🎯 Purpose

The primary objective of the ingestion pipeline is to convert unstructured documents into semantically searchable vector representations while preserving contextual integrity and metadata.
...
- Generate rich document metadata
- Split documents into semantic chunks
- Generate dense vector embeddings
- Store vectors inside PostgreSQL + pgvector
- Maintain ingestion statistics and telemetry
...
|------------|----------------|
| Loader Factory | Loads supported document formats |
| Document Parser | Extracts structured text |
| Document Cleaner | Removes noise and normalizes content |
| Metadata Pipeline | Generates searchable metadata |
| Chunking Pipeline | Creates semantic document chunks |
| Embedding Pipeline | Converts text into dense vectors |
| Vector Service | Stores vectors inside pgvector |

---

## 💡 Why It Matters

A Retrieval-Augmented Generation system is only as good as its knowledge base. High-quality chunking, metadata enrichment, and embedding generation directly improve retrieval precision and reduce hallucinations during generation.
...
The Memory Pipeline provides conversational continuity by maintaining user sessions, conversation history, and AI-generated summaries. It ensures that follow-up questions remain context-aware without repeatedly sending the entire conversation to the language model.

---

## 🎯 Purpose

To maintain long-term conversational context while optimizing token usage through intelligent summarization.

---

## 🛠 Responsibilities

- Create or load conversation sessions
...
| Component | Responsibility |
|------------|----------------|
| Session Repository | Session lifecycle management |
| Conversation Repository | Stores chat history |
| Summary Generator | Creates compressed conversation summaries |
| Memory Builder | Builds contextual prompt memory |
| Persistence Layer | Saves conversation state |

---

## 💡 Why It Matters

Without memory, every interaction becomes independent, causing the system to forget previous discussions. The Memory Pipeline enables natural multi-turn conversations while significantly reducing token consumption.
...
<p align="center">
<img src="images/retrieval.png" width="100%">
</p>

## Overview

The Retrieval Pipeline identifies the most relevant knowledge required to answer a user's question. Rather than relying on keyword matching alone, it combines semantic search, intelligent query analysis, filtering, reranking, and context construction to maximize retrieval quality.

---

## 🎯 Purpose

To retrieve highly relevant, context-rich documents that provide reliable grounding for the language model.
...
| Query Rewriter | Improves retrieval quality |
| Retrieval Engine | Performs vector similarity search |
| Filter Engine | Applies metadata constraints |
| Reranker | Orders retrieved documents |
| Context Builder | Builds LLM-ready context |

---

## 💡 Why It Matters

Retrieval quality directly determines answer quality. Better retrieval improves grounding, increases context precision, and significantly reduces hallucinations in downstream generation.
...
The Generation Pipeline converts retrieved knowledge into grounded natural language responses. It constructs optimized prompts, invokes the language model, extracts supporting citations, and formats the final response before handing it to the evaluation layer.

---

## 🎯 Purpose

To generate accurate, context-grounded, and explainable responses supported by retrieved evidence.
...
- Invoke the Groq language model
- Generate grounded responses
- Extract supporting citations
- Format references
- Assemble structured responses
...
| Prompt Manager | Combines context and instructions |
| Groq LLM | Generates grounded responses |
| Citation Extractor | Maps answers to supporting evidence |
| Response Formatter | Produces final structured output |

---

## 💡 Why It Matters

Generation is more than simply asking an LLM for an answer. By enforcing strict grounding, structured prompts, and automatic citation extraction, the system produces responses that are transparent, verifiable, and suitable for enterprise applications.

---

> **The generated response is now passed to the Evaluation Pipeline, where it undergoes grounding verification, hallucination detection, confidence scoring, and autonomous quality assessment before reaching the user.**
